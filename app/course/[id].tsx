...
import { getCourseSettings } from "../../src/features/progress/progressService";
...
const generateYouTubePlayerHTML = (videoId: string): string => {
    if (!videoId)
        return '<html><body style="background:#000;color:#fff;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;"><p>Invalid video</p></body></html>';
...
const getEmbedUrl = (
    ...
    if (provider === "youtube") {
        ...
        let videoId: string | undefined = url.match(
            /(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([^&\n?#]+)/,
        )?.[1];
    ...
    if (provider === "vimeo") {
        ...
        return videoId
            ? `https://player.vimeo.com/video/${videoId}?playsinline=1&byline=0&portrait=0&title=0`
            : null;
    }
    ...
    if (provider === "wistia") {
        ...
        return videoId
            ? `https://fast.wistia.net/embed/iframe/${videoId}?playsinline=true`
            : null;
    }
...
export default function CoursePlayerScreen() {
    ...
    const isTablet = (Platform as any).isPad || (Platform.OS === 'android' && Math.min(screenDimensions.width, screenDimensions.height) >= 600);
    const isTabletLandscape = isTablet && isLandscape;
    const isPhoneLandscape = !isTablet && isLandscape;

    // Track screen dimension changes for landscape/portrait
    useEffect(() => {
    ...
    // Track progress bar width for accurate seek calculations
    ...
    const [currentPdfLocalPath, setCurrentPdfLocalPath] = useState<string | null>(
        null,
    );
    ...
    const lessonChangeTimeout = useRef<ReturnType<typeof setTimeout> | null>(
        null,
    );
    ...
    const handleDownloadCourse = async () => {
                    ...
                    { text: "Cancel", style: "cancel" },
                    {
                        ...
                        onPress: async () => {
                            try {
                                ...
                                const { deleteOfflineCourse: deleteCourse } =
                                ...
                                Alert.alert(
                                    "Removed",
                                    "Course has been removed from offline storage.",
                                );
                            } catch (error) {
        ...
        try {
            ...
            Alert.alert(
                "Download Failed",
                error.message || "Failed to download course. Please try again.",
            );
        } finally {
    ...
    const handleFileDownload = async (url: string, filename: string) => {
        try {
            ...
            const localPath =
                (documentDirectory || cacheDirectory || "") + safeFilename;
            ...
            if (fileExists && isPdf) {
                ...
                Alert.alert("PDF Available", `"${filename}" is already downloaded.`, [
                    { text: "Cancel", style: "cancel" },
                    {
                        text: "View",
                        onPress: () => {
        ...
        try {
            ...
            const downloadResumable = (
                ...
                (downloadProgress) => {
                    ...
                    console.log(
                        `Downloading: ${filename} (${Math.round(progress * 100)}%)`,
                    );
            ...
            if (
                ...
                errStr.includes("INTERNAL_ERROR") ||
                errStr.includes("stream was reset")
            ) {
    ...
    const showSkipIndicatorAnimation = (
        side: "left" | "right",
        seconds: number,
    ) => {
                        ...
                        try {
                            const result = await downloadLessonContent(
                                ...
                                (progress, currentFile) => {
                                    console.log(
                                        `Downloading: ${currentFile} (${Math.round(progress * 100)}%)`,
                                    );
                                    setDownloadStates((prev) => {
                                        const newMap = new Map(prev);
                                        newMap.set(lessonId, {
                            ...
                            if (successCount === 0 && totalCount === 0) {
                                Alert.alert(
                                    "No Content",
                                    "This lesson has no downloadable content.",
                                );
                            } else {
                                Alert.alert(
                                    "Download Complete",
                            ...
                            Alert.alert(
                                "Download Error",
                                err.message || "Failed to download lesson",
                            );
                        }
                    },
                },
    ...
    const downloadState = currentLesson
        ? downloadStates.get(currentLesson.id)
        : null;
    ...
    const getDirectVideoSource = () => {
        ...
        if (currentLesson.video_url.startsWith("file://")) {
            console.log(
                "Using local video path from offline course:",
    ...
    const embedUrl =
        useEmbeddedPlayer && currentLesson?.video_url
    ...
    const youtubeVideoId =
        isYouTube && currentLesson?.video_url
            ...
            : null;
    const youtubePlayerHTML = youtubeVideoId
        ? generateYouTubePlayerHTML(youtubeVideoId)
        : null;
    ...
    // Helper: renders course outline panel (shared between Modal and inline)
    const renderOutlinePanel = (inline: boolean) => (
        <View style={[inline ? styles.splitOutlinePanel : styles.sidebar, { paddingBottom: inline ? 0 : insets.bottom }]}>
            ...
            <View style={styles.courseProgress}>
                <View style={styles.courseProgressBar}>
                    <View style={[styles.courseProgressFill, { width: `${((currentIndex + 1) / allLessons.length) * 100}%` }]} />
                </View>
                    ...
                    <View style={styles.moduleSection}>
                        ...
                        {(chapter.lessons || []).map((lesson: Lesson) => {
                            ...
                            return (
                                <TouchableOpacity
                                    key={lesson.id}
                                    style={[styles.sidebarLesson, isActive && styles.sidebarLessonActive]}
                                    onPress={() => selectLesson(flatIndex)}
                                >
                                    <View style={[styles.lessonStatusIcon, isCompleted && styles.lessonStatusCompleted, isActive && styles.lessonStatusActive]}>
                                        {isCompleted ? (
                                            <Ionicons name="checkmark" size={14} color="#fff" />
                                        ) : (
                                            <Ionicons name={getLessonIcon(lesson.content_type) as any} size={14} color={isActive ? "#fff" : colors.textSecondary} />
                                        )}
                                    </View>
                                    <View style={styles.sidebarLessonInfo}>
                                        <Text style={[styles.sidebarLessonTitle, isActive && styles.sidebarLessonTitleActive]} numberOfLines={2}>
                                            {lesson.title}
                                        </Text>
    ...
    return (
        <View style={[styles.container, isPhoneLandscape && styles.containerLandscape]}>
            <StatusBar
                barStyle="light-content"
                translucent
                backgroundColor="transparent"
                hidden={isPhoneLandscape}
            />
            {/* Split layout wrapper for tablet landscape */}
            <View style={isTabletLandscape ? styles.splitContainer : { flex: 1 }}>
                <View style={isTabletLandscape ? styles.splitLeftPanel : { flex: 1 }}>
                    ...
                    {currentLesson?.content_type === "video" && currentLesson?.video_url ? (
                        <View
                            style={[
                                ...
                                isPhoneLandscape && styles.mediaContainerLandscape,
                                isPhoneLandscape && {
                            ...
                            {isYouTube && youtubePlayerHTML ? (
                                ...
                                <View
                                    style={[
                                        ...
                                        isPhoneLandscape && styles.embeddedVideoWrapperLandscape,
                                        isLandscape && {
                                    ...
                                    <WebView
                                        ...
                                        renderLoading={() => (
                                            <View style={styles.embeddedLoadingOverlay}>
                                                ...
                                                <Text style={styles.embeddedLoadingText}>
                                                    Loading video...
                                                </Text>
                                            </View>
                                        )}
                                ...
                                videoError ? (
                                    ...
                                    <View
                                        style={[
                                            styles.videoWrapper,
                                            isPhoneLandscape && styles.videoWrapperLandscape,
                                            isPhoneLandscape && {
                                                width: screenDimensions.width - insets.left - insets.right,
                                                height:
                                        ...
                                        <Video
                                            ...
                                            onError={(error) => {
                                                ...
                                                if (
                                                    ...
                                                    errorStr.includes("SSLPeerUnverifiedException")
                                                ) {
                                                    errorMsg =
                                                        "SSL Certificate Error: The video server has an invalid certificate.";
                                                } else if (
                                                    errorStr.includes("404") ||
                                                    errorStr.includes("not found")
                                                ) {
                                                    errorMsg =
                                                        "Video not found. The file may have been moved or deleted.";
                                                } else if (
                                                    ...
                                                    errorStr.includes("Decoder") ||
                                                    errorStr.includes("codec") ||
                                                    errorStr.includes("c2.qti")
                                                ) {
                                                    // Hardware decoder error - suggest retry
                                                    errorMsg =
                                                        "Video decoder error. The video format may not be supported. Try again or contact support.";
                                                }
                                            ...
                                            <View style={styles.controlsOverlay} pointerEvents="box-none">
                                                ...
                                                <View
                                                    style={[
                                                        ...
                                                        isPhoneLandscape && styles.bottomControlsLandscape,
                                                    ]}
                                                >
                                                    ...
                                                    <View
                                                        style={[
                                                            ...
                                                            isPhoneLandscape && styles.progressContainerLandscape,
                                                        ]}
                                                    >
                                                        <View
                                                    ...
                                                    >
                                                        ...
                                                        <View style={styles.bottomRightControls}>
                                                            <TouchableOpacity
                                                                ...
                                                                onPress={() =>
                                                                    seekVideo(Math.max(0, videoProgress - 10000))
                                                                }
                                                            ...
                                                            <TouchableOpacity
                                                                ...
                                                                onPress={() =>
                                                                    seekVideo(
                                                                        Math.min(videoDuration, videoProgress + 10000),
                                                                    )
                                                                }
                                                                hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                                                            >
                    ...
                    <View style={styles.contentArea}>
                            ...
                            <View style={styles.courseDownloadBanner}>
                                <View style={styles.courseDownloadInfo}>
                                    ...
                                    <View style={styles.courseDownloadTexts}>
                                        <Text style={styles.courseDownloadTitle}>
                                            Downloading Course...
                                        </Text>
                                    ...
                                    <Text style={styles.courseDownloadPercent}>
                                        {Math.round(courseDownloadProgress * 100)}%
                                    </Text>
                                </View>
                                <View style={styles.courseDownloadProgressBg}>
                                    <View
                                        style={[
                                            styles.courseDownloadProgressFill,
                                            { width: `${courseDownloadProgress * 100}%` },
                                        ]}
                                    />
                                </View>
                            </View>
                        ...
                        <View style={styles.lessonHeader}>
                            <View style={styles.lessonInfo}>
                                <Text style={styles.moduleLabel}>
                                    ...
                                    {(currentLesson?.lessonIndex || 0) + 1}/
                                    {currentLesson?.totalInModule}
                                </Text>
                                                ...
                                                {block.block_type === "text" &&
                                                    (block.content?.html ||
                                                        ...
                                                        <View style={styles.textBlockContainer}>
                                                            ...
                                                            <RenderHtml
                                                                contentWidth={
                                                                    SCREEN_WIDTH - Theme.spacing.lg * 2 - 32
                                                                }
                                                                source={{
                                                                    html:
                                                                        block.content?.html ||
                                                ...
                                                {block.block_type === "quiz" &&
                                                    !(
                                                        ...
                                                        <View style={styles.inlineQuizCard}>
                                                            ...
                                                            <TouchableOpacity
                                                                ...
                                                                onPress={() => {
                                                                    ...
                                                                    const preparedQuiz = {
                                                                        id: block.id,
                                                                        title:
                                                                            quizContent.title || block.title || "Quiz",
                                                                        description: "Test your knowledge",
                                                            ...
                                                            >
                                                                <Text style={styles.inlineQuizButtonText}>
                                                                    Take Quiz
                                                                </Text>
                                                                <Ionicons
                                                                    name="arrow-forward"
                        ...
                        >
                            ...
                            {currentIndex === allLessons.length - 1 ? (
                                ...
                                >
                                    <Text
                                        style={[styles.navButtonText, styles.navButtonTextComplete]}
                                    >
                                        Complete
                                    </Text>
                ...
                </View>{/* end splitLeftPanel */}
                {isTabletLandscape && renderOutlinePanel(true)}
            </View>{/* end splitContainer */}

            {/* Course Outline Sidebar (Modal) - only for non-tablet-landscape */}
            {!isTabletLandscape && (
                <Modal
                    visible={showSidebar}
                    animationType="slide"
                                                                ...
                                                                {dState?.isDownloading && (
                                                                    <View style={styles.downloadStatusContainer}>
                                                                        ...
                                                                        <Text style={styles.downloadProgressText}>
                                                                            {Math.round(dState.progress * 100)}%
                                                                        </Text>
                                                                    </View>
                                                                )}
                                                                {dState?.isDownloaded &&
    ...
    <script>
        ...
        initPdf();
                            ...
                            <View style={styles.pdfEmptyState}>
                                ...
                                <Text style={styles.pdfEmptySubtext}>
                                    Download a PDF first to view it offline
                                </Text>
                            </View>
                        )}
                    </View>
...
function createStyles(colors: typeof Theme.colors.light, isDark: boolean) {
    return StyleSheet.create({
        ...
        // Landscape: Fill entire screen without aspect ratio constraint
        ...
        // Landscape mode - add safe area padding for notch/navigation
        ...
        bottomGradient: {
            ...
            left: 0,
            right: 0,
            height: 140, // Increased gradient height for better visibility of controls
            zIndex: 25,
        },
        bottomControls: {
            paddingHorizontal: 12,
        ...
        // Landscape progress container - more padding for safe areas
        ...
        timeRow: {
            ...
            paddingHorizontal: 12,
            paddingTop: 6,
            paddingBottom: 12, // Increased bottom padding to ensure visibility
            zIndex: 30,
        },
        timeText: {
        ...
        // Split layout styles for tablet landscape
        splitContainer: {
            flex: 1,
